...
// TEMP define for testing
//#define USING_TGA_ORIGINAL

#ifdef USING_TGA_ORIGINAL
...
public:
	...
	CAnimationController(const char *modelpath) 
		: m_NumBones(0)
		, _curScene(NULL)
	...
	void BoneTransform(std::vector<aiMatrix4x4>& Transforms)
	{
		...
		{
			float TicksPerSecond = static_cast<float>(_scenes[_curScene]->mAnimations[0]->mTicksPerSecond) != 0 ? 
				static_cast<float>(_scenes[_curScene]->mAnimations[0]->mTicksPerSecond) : 25.0f;
			float TimeInTicks = _animationTime0 * TicksPerSecond;
