...
CScene* CSceneManager::CreateScene(std::string aJsonFile)
{



	rapidjson::Document document = CJsonReader::LoadDocument(ASSETPATH + "Assets/" +  aJsonFile);
	...
	for (auto& jsongameobject : jsonarray) {
		...
		for (auto& jsonTransform : jsonTransforms) {
			...
			instancedTransforms.emplace_back(transform.GetWorldMatrix());
