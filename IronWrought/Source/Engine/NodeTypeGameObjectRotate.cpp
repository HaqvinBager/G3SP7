...
int CNodeTypeGameObjectRotate::OnEnter(CNodeInstance* aTriggeringNodeInstance)
{
	...
	float x = NodeData::Get<float>(someData) * CTimer::Dt() * 3.14159265f / 180.0f;
	...
	DirectX::SimpleMath::Quaternion rotation = DirectX::SimpleMath::Quaternion::CreateFromYawPitchRoll(y, x, z);
