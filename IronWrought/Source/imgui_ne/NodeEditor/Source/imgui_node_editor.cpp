...
ed::EditorAction::AcceptResult ed::ShortcutAction::Accept(const Control& control)
{
    ...
    if (io.KeyCtrl && !io.KeyShift && !io.KeyAlt && ImGui::IsKeyPressed(ImGui::GetKeyIndex(ImGuiKey_Z)))
        candidateAction = Undo;
    if (io.KeyCtrl && !io.KeyShift && !io.KeyAlt && ImGui::IsKeyPressed(ImGui::GetKeyIndex(ImGuiKey_Y)))
        candidateAction = Redo;

    if (candidateAction != None)
    {
        if (candidateAction != Paste && candidateAction != CreateNode && candidateAction != Undo && candidateAction != Redo)
        {
...
bool ed::ShortcutAction::AcceptUndo()
{
    IM_ASSERT(m_InAction);
    return m_CurrentAction == Undo;
}

bool ed::ShortcutAction::AcceptRedo()
{
    IM_ASSERT(m_InAction);
