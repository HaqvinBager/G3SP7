...
namespace ImGui
{
    ...
    int Curve(const char *label, const ImVec2& size, const int maxpoints, ImVec2 *points)
    {
        ...
        for (i = 1; i < max; i++)
        {
            ...
            window->DrawList->AddLine(a, b, GetColorU32(ImGuiCol_PlotLinesHovered, 0.25f));
        }
    ...
    int IronCurve(const char* label, ImVec2* points, const ImVec2& size = { 200.0f, 200.0f }, const int maxpoints = 4)
    {
        int modified = 0;
        int i;
        if (maxpoints < 2 || points == 0)
